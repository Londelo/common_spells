...
import { yellow, red, green } from '../shared/colors'
...
async function nav() {
  ...
  if (args[0] === '--config' && args[1]) {
    ...
    echo(green(`Navigation base directory set to: ${args[1]}`))
    return
  }
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '-o' || args[i] === '--open') openAfter = true
    else keyword = args[i]
  }
...
(async () => await errorHandlerWrapper(nav, errorMessage))()
